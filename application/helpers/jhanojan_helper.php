...
function is_login(){
	$CI =& get_instance();
        if(!$CI->session->userdata('webmaster_id'))
            redirect('webpanel/login');
}
function is_login_web(){
	$CI =& get_instance();
        if($CI->session->userdata('webmember_id'))
            return true;
			else 
			return false;
}
function must_login_web(){
	$CI =& get_instance();
        if(!$CI->session->userdata('webmember_id'))
...
function cekLoginweb($username,$userpass)
{
	$CI =& get_instance();
	$CI->db->where("email",$username);
	$CI->db->where("password",$userpass);
	$CI->db->where("approved",'1');
	$query=$CI->db->get("sv_pic");
	return $query;
}
	...
	function permission()
	{
		...
		if($group != "8910")
		{
			...
			if($jum > 0)
			{
				...
				if($jum_menu_admin == 0)
				{
					redirect("webpanel/forbiden");
				}
			}
			else redirect("webpanel/forbiden");
		}
	...
	function GetAll($tbl,$filter=array(),$filter_where_in=array(),$filter_where_not_in=array())
	{
		...
		foreach($filter as $key=> $value)
		{
			...
			if(isset($exp[1]))
			{
				...
				if($exp[0] == "where_null") $CI->db->where("$key IS NULL");
				if($exp[0] == "where") $CI->db->where($key, $exp[1]);
...
function reg_periode($exit=true,$id=1){
	date_default_timezone_set("Asia/Jakarta");
	$now=date('Y-m-d');
	$CI=&get_instance();
	if($CI->session->userdata('webmember_onsites'))return true;
	$prd=GetAll('reg_periode',array('id'=>'where/'.$id))->row_array();
	if($now>=$prd['start_date'] && $now<=$prd['end_date']){
		return true;
	}else{
		if($exit){
			echo '<script type="text/javascript">alert("Periode Pendaftaran Telah Ditutup");history.go(-1);</script>';
			exit();
		}else{
			return false;
		}
...
function send_email_($member,$body,$subject=''){
	$CI=&get_instance();
	//$config['useragent'] = 'CodeIgniter';
	$config['protocol'] = 'smtp';
	//$config['mailpath'] = '/usr/sbin/sendmail';
	$config['smtp_host'] = 'ssl://g20registrasiktt.id';
	
	$config['smtp_user'] = 'panitia@g20registrasiktt.id';
	$config['smtp_pass'] = '1Convexpacto#';
	...
	$bcc=array('ojanevent@gmail.com','made.suardana@pactoconvex.com');
	$body = $CI->load->view($body,$data,TRUE);
	$CI->load->library('email'); 
	$CI->email->initialize($config);
	$CI->email->from('panitia@g20registrasiktt.id','Panitia Registrasi KTT G20');
	$CI->email->to($to);
	$CI->email->bcc($bcc);
...
function send_email_member($member,$body,$subject){
	$CI=&get_instance();
	$config['protocol'] = 'smtp';
	$config['smtp_host'] = 'ssl://g20registrasiktt.id';
	
	$config['smtp_user'] = 'panitia@g20registrasiktt.id';
	//$config['smtp_pass'] = '3gI7qv3#9';
	$config['smtp_pass'] = '1Convexpacto#';
	$config['smtp_port'] = 465; 
	$config['smtp_timeout'] = 30;
	$config['wordwrap'] = TRUE;
	...
	$config['bcc_batch_size'] = 200;
	$data['member']=$member;
	$to=array(GetValue('email','pic',array('id'=>'where/'.$member[0]['pic'])));
	$bcc=array('ojanevent@gmail.com','made.suardana@pactoconvex.com');
	$body = $CI->load->view($body,$data,TRUE);
	$CI->load->library('email'); 
	$CI->email->initialize($config);
...
function send_email_akomodasi($member,$body,$subject){
	$CI=&get_instance();
	$config['protocol'] = 'smtp';
	$config['smtp_host'] = 'ssl://g20registrasiktt.id';
	...
	$config['bcc_batch_size'] = 200;
	$data['member']=$member;
	$to=array(GetValue('email','pic',array('intansi'=>'where/'.$member[0]['intansi'],'pic_type'=>'where/3')));
	$bcc=array('ojanevent@gmail.com','made.suardana@pactoconvex.com');
	$body = $CI->load->view($body,$data,TRUE);
	$CI->load->library('email'); 
	$CI->email->initialize($config);
...
function getcompanyname(){
    $CI=&get_instance();
    $ncompany=$CI->db->query("SELECT name FROM tb_setup_company WHERE id=1")->row_array();
    return $ncompany['name'];
}
?>